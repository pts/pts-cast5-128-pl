...
Crypt::CAST5_128_PP - CAST5 block cipher with 128-bit key in pure Perl
...
This module has a simplified implementation which supports key size of 128
bits only. Use Crypt::CAST5_128_PP if you need all key sizes (40 .. 128
bits, divisible by 8).
...
For a quick smoke test, run:
    ...
    $ perl -I. -mCrypt::CAST5_128_PP=test -e0
    Crypt::CAST5_128_PP OK.

=head1 HISTORY

Based on Crypt::CAST5_PP version 1.04.

=cut

package Crypt::CAST5_128_PP;
require 5.004;
...
sub init {
  my ($cast5, $key) = @_;
  croak "Key length must be 16 bytes" if length($key) != 16;
  my ($s, $t, $u, $v) = unpack "N4", $key;  # Split to uint32s.
  my ($w, $x, $y, $z, @ks);
  for (1..2) {
    ...
    push@ks,$s5[$y>>24&255]^$s6[$y>>16&255]^$s7[$x&255]^$s8[$x>>8&255]^$s5[$w>>8&255];
    push@ks,$s5[$y>>8&255]^$s6[$y&255]^$s7[$x>>16&255]^$s8[$x>>24&255]^$s6[$x>>8&255];
    push@ks,$s5[$z>>24&255]^$s6[$z>>16&255]^$s7[$w&255]^$s8[$w>>8&255]^$s7[$y>>16&255];
    push@ks,$s5[$z>>8&255]^$s6[$z&255]^$s7[$w>>16&255]^$s8[$w>>24&255]^$s8[$z>>24&255];
    ...
    push@ks,$s5[$s&255]^$s6[$s>>8&255]^$s7[$v>>24&255]^$s8[$v>>16&255]^$s5[$u>>24&255];
    push@ks,$s5[$s>>16&255]^$s6[$s>>24&255]^$s7[$v>>8&255]^$s8[$v&255]^$s6[$v>>16&255];
    push@ks,$s5[$t&255]^$s6[$t>>8&255]^$s7[$u>>24&255]^$s8[$u>>16&255]^$s7[$s&255];
    push@ks,$s5[$t>>16&255]^$s6[$t>>24&255]^$s7[$u>>8&255]^$s8[$u&255]^$s8[$t&255];
    $w=$s^$s5[$v>>16&255]^$s6[$v&255]^$s7[$v>>24&255]^$s8[$v>>8&255]^$s7[$u>>24&255];
    $x=$u^$s5[$w>>24&255]^$s6[$w>>8&255]^$s7[$w>>16&255]^$s8[$w&255]^$s8[$u>>8&255];
    $y=$v^$s5[$x&255]^$s6[$x>>8&255]^$s7[$x>>16&255]^$s8[$x>>24&255]^$s5[$u>>16&255];
    $z=$t^$s5[$y>>8&255]^$s6[$y>>16&255]^$s7[$y&255]^$s8[$y>>24&255]^$s6[$u&255];
    push@ks,$s5[$w&255]^$s6[$w>>8&255]^$s7[$z>>24&255]^$s8[$z>>16&255]^$s5[$y>>16&255];
    push@ks,$s5[$w>>16&255]^$s6[$w>>24&255]^$s7[$z>>8&255]^$s8[$z&255]^$s6[$z>>24&255];
    push@ks,$s5[$x&255]^$s6[$x>>8&255]^$s7[$y>>24&255]^$s8[$y>>16&255]^$s7[$w>>8&255];
    push@ks,$s5[$x>>16&255]^$s6[$x>>24&255]^$s7[$y>>8&255]^$s8[$y&255]^$s8[$x>>8&255];
    $s=$y^$s5[$x>>16&255]^$s6[$x&255]^$s7[$x>>24&255]^$s8[$x>>8&255]^$s7[$w>>24&255];
    $t=$w^$s5[$s>>24&255]^$s6[$s>>8&255]^$s7[$s>>16&255]^$s8[$s&255]^$s8[$w>>8&255];
    $u=$x^$s5[$t&255]^$s6[$t>>8&255]^$s7[$t>>16&255]^$s8[$t>>24&255]^$s5[$w>>16&255];
    $v=$z^$s5[$u>>8&255]^$s6[$u>>16&255]^$s7[$u&255]^$s8[$u>>24&255]^$s6[$w&255];
    push@ks,$s5[$u>>24&255]^$s6[$u>>16&255]^$s7[$t&255]^$s8[$t>>8&255]^$s5[$s&255];
    push@ks,$s5[$u>>8&255]^$s6[$u&255]^$s7[$t>>16&255]^$s8[$t>>24&255]^$s6[$t&255];
    push@ks,$s5[$v>>24&255]^$s6[$v>>16&255]^$s7[$s&255]^$s8[$s>>8&255]^$s7[$u>>24&255];
    push@ks,$s5[$v>>8&255]^$s6[$v&255]^$s7[$s>>16&255]^$s8[$s>>24&255]^$s8[$v>>16&255];
  }
  for (16..31) { $ks[$_] &= 31 }
  $cast5->{ks} = \@ks;
  return $cast5;
...
sub encrypt {
  croak "Block size must be 8" if length($_[1]) != 8;
  my($ks,$l,$r,$i)=($_[0]{ks},unpack"N2",$_[1]);
  $i=$ks->[0]+$r; $i=$i<<$ks->[16]|$i>>(32-$ks->[16])&(~(~0<<$ks->[16])&0xffffffff); $l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$ks->[1]^$l; $i=$i<<$ks->[17]|$i>>(32-$ks->[17])&(~(~0<<$ks->[17])&0xffffffff); $r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$ks->[2]-$r; $i=$i<<$ks->[18]|$i>>(32-$ks->[18])&(~(~0<<$ks->[18])&0xffffffff); $l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$ks->[3]+$l; $i=$i<<$ks->[19]|$i>>(32-$ks->[19])&(~(~0<<$ks->[19])&0xffffffff); $r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$ks->[4]^$r; $i=$i<<$ks->[20]|$i>>(32-$ks->[20])&(~(~0<<$ks->[20])&0xffffffff); $l^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$ks->[5]-$l; $i=$i<<$ks->[21]|$i>>(32-$ks->[21])&(~(~0<<$ks->[21])&0xffffffff); $r^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$ks->[6]+$r; $i=$i<<$ks->[22]|$i>>(32-$ks->[22])&(~(~0<<$ks->[22])&0xffffffff); $l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$ks->[7]^$l; $i=$i<<$ks->[23]|$i>>(32-$ks->[23])&(~(~0<<$ks->[23])&0xffffffff); $r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$ks->[8]-$r; $i=$i<<$ks->[24]|$i>>(32-$ks->[24])&(~(~0<<$ks->[24])&0xffffffff); $l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$ks->[9]+$l; $i=$i<<$ks->[25]|$i>>(32-$ks->[25])&(~(~0<<$ks->[25])&0xffffffff); $r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$ks->[10]^$r; $i=$i<<$ks->[26]|$i>>(32-$ks->[26])&(~(~0<<$ks->[26])&0xffffffff); $l^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$ks->[11]-$l; $i=$i<<$ks->[27]|$i>>(32-$ks->[27])&(~(~0<<$ks->[27])&0xffffffff); $r^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$ks->[12]+$r; $i=$i<<$ks->[28]|$i>>(32-$ks->[28])&(~(~0<<$ks->[28])&0xffffffff); $l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$ks->[13]^$l; $i=$i<<$ks->[29]|$i>>(32-$ks->[29])&(~(~0<<$ks->[29])&0xffffffff); $r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$ks->[14]-$r; $i=$i<<$ks->[30]|$i>>(32-$ks->[30])&(~(~0<<$ks->[30])&0xffffffff); $l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$ks->[15]+$l; $i=$i<<$ks->[31]|$i>>(32-$ks->[31])&(~(~0<<$ks->[31])&0xffffffff); $r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  pack"N2",$r&0xffffffff,$l&0xffffffff
}

sub decrypt {
  croak "Block size must be 8" if length($_[1]) != 8;
  my($ks,$r,$l,$i)=($_[0]{ks},unpack"N2",$_[1]);
  $i=$ks->[15]+$l; $i=$i<<$ks->[31]|$i>>(32-$ks->[31])&(~(~0<<$ks->[31])&0xffffffff); $r^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  $i=$ks->[14]-$r; $i=$i<<$ks->[30]|$i>>(32-$ks->[30])&(~(~0<<$ks->[30])&0xffffffff); $l^=(($s1[$i>>24&255]+$s2[$i>>16&255])^$s3[$i>>8&255])-$s4[$i&255];
  $i=$ks->[13]^$l; $i=$i<<$ks->[29]|$i>>(32-$ks->[29])&(~(~0<<$ks->[29])&0xffffffff); $r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$ks->[12]+$r; $i=$i<<$ks->[28]|$i>>(32-$ks->[28])&(~(~0<<$ks->[28])&0xffffffff); $l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  ...
  $i=$ks->[1]^$l; $i=$i<<$ks->[17]|$i>>(32-$ks->[17])&(~(~0<<$ks->[17])&0xffffffff); $r^=(($s1[$i>>24&255]-$s2[$i>>16&255])+$s3[$i>>8&255])^$s4[$i&255];
  $i=$ks->[0]+$r; $i=$i<<$ks->[16]|$i>>(32-$ks->[16])&(~(~0<<$ks->[16])&0xffffffff); $l^=(($s1[$i>>24&255]^$s2[$i>>16&255])-$s3[$i>>8&255])+$s4[$i&255];
  pack"N2",$l&0xffffffff,$r&0xffffffff
}

sub test {
  my $class = ref($_[0]) || $_[0];
  my $cb = $_[0]->new('ThisIsMyCAST5Key');
  die unless "@{$cb->{ks}}" eq '947271624 2179958043 2182555503 73893026 2171948028 2297882293 4091083205 437067796 966602889 552282757 3156510989 3774056109 3329389301 3661444638 1475472839 3255644729 11 29 23 31 5 30 14 17 24 1 18 13 28 4 10 15';
  my $c1 = $cb->encrypt('Hello, W');
  die unless unpack("H*", $c1) eq 'c42785396cbc6e62';
  die unless $cb->decrypt($c1) eq 'Hello, W';
  my $c2 = $cb->encrypt('orld!CA5');
  die unless unpack("H*", $c2) eq 'edb961e4ab7d268f';
  die unless $cb->decrypt($c2) eq 'orld!CA5';
  print STDERR "$class OK.\n";
}

sub import {
  $_[0]->test() if @_ > 1 and $_[1] eq 'test';
}
